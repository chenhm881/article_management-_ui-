        ...
        return (
            <div style={{marginTop: "5%"}}><Form ref={this.formRef} {...layout}
              ...
              fields={[
                  {
                      name: ['article', 'tags'],
                      value: (article.tags instanceof Array ? article.tags.map((tag: { id: any; }) => ( tag.id)) : undefined)
                  },
                  {
                      name: ['article', 'title'],
                      value: article.title,
            ...
            <Form.Item name={['article', 'categoryId']} label="Category" rules={[{ required: true }]}>
                <Select
                    ...
                    options={listCategory.map(({categoryId, categoryName}: any) => ({value: categoryId, label: categoryName}))}
                />
            </Form.Item>
            <Form.Item name={['article', 'tags']} label="Tags">
                <Select
                    ...
                    options={listTag.map(({id, tagName}: any) => ({value: id, label: tagName}))}
                />
            </Form.Item>
